...
Patrasche 백엔드는 FastAPI 기반의 여러 서비스로 나뉘어 있습니다. 사용자 요청 처리, 메일 발송, RSS 수집, 뉴스 요약/저장을 각각 독립된 서비스가 담당합니다.
...
또한 네 개 서비스에 대해 flake8을 실행하며, 현재 기준 line length는 130으로 맞춰져 있습니다.

## Docker 빌드

Docker build context는 `backend/` 디렉터리입니다. Dockerfile은 `backend/` 바로 아래에 있습니다.
...
운영 환경의 DB, SMTP, 내부 token, 외부 API key 값은 코드에 저장하지 않습니다. 인프라 레포의 SSM Parameter Store와 External Secrets Operator 설정을 통해 Kubernetes Secret으로 주입받는 구조입니다.
